...
const Dropdown = ({ items, mobile }) => {
  ...
  return (
    ...
    >
      ...
      >
        {items.name}
        {items.children.length > 0 && <div className="border-l pl-2 border-gray-600 md:border-none"><ChevronDown size={25} /></div>}
      </button>
      {items.children.length > 0 && open && (
        <motion.div
          ...
          className={`absolute left-0 mt-1 bg-gray-100 shadow-lg rounded-lg py-2 border border-gray-300 backdrop-blur-lg bg-opacity-90 ${mobile ? 'relative mt-0 bg-gray-950 border-none ' : ''}`}
        >
          {items.children.map((child, index) => (
            <div key={`children-${index}`} className="relative  flex">
              <a href={child.link} className={`block px-4 w-50 py-2 text-gray-700 hover:bg-gray-200 cursor-pointer ${mobile ? 'text-white' : ''}`}>
                {child.name}
              </a>
              {
                 child.children && <div
                  className={`border-l pl-2 border-gray-600 md:border-none ${mobile? 'text-white':''} `}><ChevronRight size={25} /></div>
              }
            </div>
          ))}
        </motion.div>
